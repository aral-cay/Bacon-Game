 ...
 * Purpose: This program implements the Kevin Bacon Game, a graph-based game
 *  * that finds the shortest connection paths between actors based on their
 *  * co-appearances in movies. It uses graph algorithms to determine actor
 *  * connections, calculates average separations, and provides various statistics
 *  * and functionalities, such as changing the "center of the acting universe,"
 *  * listing actors based on degree or separation, and identifying unreachable actors.
 *  */
...
public class BaconGame {
    ...
    public void setNewUniverseCenter(String newRoot){
        if (baconGraph.hasVertex(newRoot)){
            root = newRoot;
            pathTree = GraphLibrary.bfs(baconGraph, newRoot);
    ...
    public double getAverageSeparationForRoot(String newRoot){
        Graph<String, Set<String>> tempPathTree = GraphLibrary.bfs(baconGraph, newRoot);
        return GraphLibrary.averageSeparation(tempPathTree, newRoot);
    }
    ...
    public void dispSortedBySeparation(int low, int high){
        class SeparationComparator implements Comparator<String>{
            public int compare(String actor1, String actor2) {
                return GraphLibrary.getPath(pathTree, actor1).size() - GraphLibrary.getPath(pathTree, actor2).size();
            }
        }
        ...
        for (String vertex: pathTree.vertices()){
            if (GraphLibrary.getPath(pathTree, vertex).size() > low && GraphLibrary.getPath(pathTree, vertex).size() <= high){
    ...
    public void dispInfinitelySeparated(){
        System.out.println(GraphLibrary.missingVertices(baconGraph, pathTree));
    }
    ...
    public void dispActorInfo(String actor){
        if (baconGraph.hasVertex(actor) && pathTree.hasVertex(actor)){
            List<String> shortestPath = GraphLibrary.getPath(pathTree, actor);
    ...
    public void dispUniverseCenterMessage(){
        System.out.println(root + " is now the center of the acting universe, connected to " + pathTree.numVertices() + "/" + baconGraph.numVertices() +
                " actors with average separation " + GraphLibrary.averageSeparation(pathTree, root));
    }
    ...
    public void configurePathTree(){
        pathTree = GraphLibrary.bfs(baconGraph, root);
    }
